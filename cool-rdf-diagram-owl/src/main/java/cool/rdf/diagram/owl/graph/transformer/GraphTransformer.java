...
public abstract class GraphTransformer implements UnaryOperator<Set<GraphElement>> {
   ...
   protected Stream<Node> findNodesWithIri( final Set<GraphElement> graph, final IRI iri ) {
      return graph.stream()
            ...
            .filter( node -> node.id().iri().map( nodeIri -> nodeIri.equals( iri ) ).orElse( false ) );
   }

   /**
    * Calculates a {@link ChangeSet} for a given graph in which all edges pointing to a given node are
    * instead now
   ...
   /**
    * Calculates a {@link ChangeSet} for a given graph in which all edges outgoing from a given node
    * are instead now
