...
/**
 * A mapper that translates SWRL rules into a {@link Graph}: The rule is represented as one distinct
 * node that contains
 * a human-readable string representation of the rule, with outgoing edges to the nodes representing
 * the elements
...
public class SWRLObjectMapper implements SWRLObjectVisitorEx<Graph> {
    ...
    * Edges) are collected. The values of the Literal nodes are concatenated in the end to render the
    * final rule representation. In order to differentiate the to-be-concatenated Literal nodes from
    * "regular" Literal nodes that might occur, they are given an internal identifier "marker" IRI.
