// -*- fill-column: 100; -*-
= `cool` command line tool

== Installation

=== Requirements

* To run the `cool` command line tool, you need Java 25 or later.

* Additional, optional dependencies:

** https://www.graphviz.org/[Graphviz] for the xref:cool-cli.adoc#diagram-command[generation of diagrams]
*** Install on Windows (using https://chocolatey.org/[Chocolatey]): `choco install graphviz`
*** Install on MacOs (using https://brew.sh/[Brew]): `brew install graphviz`
*** Install on Ubuntu/Debian Linux: `apt install graphviz`

=== Download and Install `cool`

The latest version is *{release-version}*. Download here: *https://github.com/cool-rdf/cool-rdf/releases/download/{release-tag}/cool-rdf-cli-{release-version}.jar[cool-rdf-cli-{release-version}.jar]*

You can find older releases at the https://github.com/cool-rdf/cool-rdf/releases/[Github releases].

[NOTE]
====
On MacOs and Linux, you can use the following convenience script to download the `cool` .jar file
and install it as `cool` command in your shell.

[source,shell,subs="attributes+,+quotes"]
----
curl -L -o ~/.local/share/cool-rdf/cool-rdf-cli.jar --create-dirs https://github.com/cool-rdf/cool-rdf/releases/download/{release-tag}/cool-rdf-cli-{release-version}.jar
echo 'function cool() { java -jar ~/.local/share/cool-rdf/cool-rdf-cli.jar $*; }' >> ~/.bashrc
source ~/.bashrc
----
====

After installation, you can use the `cool` command.

== Usage

You call `cool` (or alternatively `java -jar cool-rdf-cli-{release-version}.jar`) with subcommands and arguments.
...
General usage: `cool diagram [options] input [output]`, where `input` can be
...
General usage: `cool write [options] input [output]`, where `input` can be either an RDF file, a URL
...
General usage: `cool infer input [output]`, where `input` can be either an ontology file, an URL or
