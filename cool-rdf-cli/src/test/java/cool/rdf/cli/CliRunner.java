...
/**
 * Convenient test runner to execute a command line interface either via main class, executable jar
 * or native binary.
 * Arguments and stdin can be provided, return code, stderr and stdout are captured. This class uses
 * Mockito to capture
 * calls to {@link Runtime#exit(int)}: It relies on the tested class having a non-final field
 * "java.lang.Runtime
...
public class CliRunner {
   ...
   public record ExecArguments(
         List<String> arguments, StreamContent stdin, File workingDirectory
   ) {
   ...
   public record Result(
         int exitStatus, StreamContent stdOut, StreamContent stdErr
   ) {}
   ...
   private record StreamCollector(
         InputStream in
   ) implements Callable<ByteArrayOutputStream> {
      @Override
   ...
   /**
    * Executes a command line tool via its main class. This also captures any System.exit() calls that
    * might be done.
    *
