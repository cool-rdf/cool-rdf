...
public class CoolWrite extends AbstractCommand implements Runnable {
   ...
   private String wellKnownUriByPrefixOrElse( final String prefix, final String uri ) {
      if ( !uri.equals( fallbackUri ) ) {
         return uri;
      }
      // When we end up here, a prefix was given without a URI, i.e., this is supposed to be a
      // "well known" prefix.
      return Arrays.stream( Prefixes.values() )
   ...
   private Set<RdfPrefix> buildKnownPrefixes( final Map<String, URI> prefixes ) {
      final Set<RdfPrefix> result = prefixes.entrySet().stream().map( entry -> {
         final String uri = wellKnownUriByPrefixOrElse( entry.getKey(), entry.getValue().toString() );
         return new DefaultRdfPrefix( entry.getKey(), uri );
      ...
      protected String buildResourceUri( final String resourceUri ) {
         for ( final Map.Entry<String, URI> entry : prefixMap.entrySet() ) {
            ...
            if ( resourceUri.startsWith( entry.getKey() ) ) {
               return uri + resourceUri.substring( ( entry.getKey() + ":" ).length() );
            }
         }
