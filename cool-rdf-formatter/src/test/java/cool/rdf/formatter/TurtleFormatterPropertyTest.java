...
public class TurtleFormatterPropertyTest {
   ...
   // as there is no variation in their serialization, but greatly reduces the test space
   @Provide
   ...
   Arbitrary<Literal> anyLiteral() {
      return Arbitraries.oneOf( List.of(
            anyStringLiteral(), anyLangStringLiteral(), anyFloatLiteral(),
   ...
   // Creates styles for some of the options that control the actual formatting and are difficult to
   // test
   // in isolation.
   @Provide
   Arbitrary<FormattingStyle> anyStyle() {
      ...
      return Combinators.combine( onOff, onOff, onOff, onOff, onOff )
            .as( ( firstPredicateInNewLine, useAForRdfType, useComma, alignPredicates, alignObjects ) -> FormattingStyle.builder()
   ...
   @Property( tries = 500,
      seed = "1" )
   public void anyModelContainingSpecialCharactersIsSyntacticallyValid(
         @ForAll( "anyStringLiteralWithSpecialCharacters" ) final Literal literal ) {
      final Model model = ModelFactory.createDefaultModel();
